...
# Configuration
filename = "data/even_labeled.csv"  # Set your CSV file name here
time_speedup = 0.5  # Speed up the time by 4x
fade_time = 60  # Points disappear after 10 seconds of simulated time
...
sc = ax.scatter([], [], c=[], alpha=1, cmap='viridis')

# Function to update the plot
def update(frame):
    current_time = frame / time_speedup  # Simulated time at 4x speed
    ...
    if len(alpha_values[alpha_values > 0]) > 0:
        ...
        # print(alphas)
        # sc = ax.scatter(visible_data['y'], -visible_data['x'])

        # Update scatter plot
        sc.set_offsets(np.c_[visible_data['y'], -visible_data['x']])  # Flipped X values
        # sc.set_array(visible_data['time'])  # Color based on time
        sc.set_color(colors)
        # sc.set_array(visible_data['time'])  # Color based on time
...
frames = int(total_time * time_speedup)  # Frames for 4x speed
